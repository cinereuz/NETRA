...
function tampilHasil(data) {
  ...
  el.resultConfidence.textContent = data.confidence != null ? `Confidence: ${parseFloat(data.confidence).toFixed(1)}%` : '';

  el.resultMethod.textContent = data.method ? `Terdeteksi via: ${data.method}` : '';
...
const KATEGORI_BERBAHAYA = ['phishing', 'judi_online'];
  ...
  try {
    ...
    if (response.ok) {
      if (data.status === 'duplikat') {
        ...
        const perluTutupTab = KATEGORI_BERBAHAYA.includes(kategori);
        if (perluTutupTab) {
          tampilNotif('✅ Laporan terkirim! Tab akan ditutup... 🛡️', 'success', 2000);
          setTimeout(async () => {
...
async function muatStats() {
  try {
    ...
    el.footerStats.textContent = `📊 ${data.total_prediksi || 0} prediksi | ${data.total_laporan_user || 0} laporan`;
  } catch {
